...
async def view_single_file_content(request: Request, owner: str, repo_name: str, path: str = Query(...), github_pat: Optional[str] = Depends(get_github_pat)):
    ...
    try:
        ...
        else: download_url = file_item_response.get("download_url");_ = error_message_for_template if download_url else f'File "{escape_html_chars(os.path.basename(file_path_unquoted))}" found, but no download URL.'
...
async def initiate_cv_generation(request: Request, github_pat: Optional[str] = Depends(get_github_pat), action_type: str = Form(...), selected_repo_names: Optional[List[str]] = Form(None)):
    ...
    if not all_fetched_repos:
        ...
        if fetch_result["error"] or not fetch_result["data"]: return RedirectResponse(url=f"/repos?error={quote(f'Could not fetch repositories: {escape_html_chars(fetch_result.get(\"error\", \"No data\"))}')}", status_code=303)
        all_fetched_repos = fetch_result["data"]; app.state.user_repos_cache = all_fetched_repos
...
async def _cv_event_stream_generator(
    process_id: str, 
    github_pat_token: str, # Renamed to avoid conflict with outer scope var if any
    app_state_tasks: dict # Pass the tasks dict for updates
) -> AsyncGenerator[str, None]:
    """Inner async generator for SSE events."""
    
    task_data = app_state_tasks.get(process_id) # Get a reference to the specific task's data

    # These checks are now part of this generator, which StreamingResponse will consume
    if not github_pat_token: # Should be caught by `initiate_cv_generation` but double check
        yield f"data: {json.dumps({'type': 'error', 'message': 'Authentication context lost.'})}\n\n"
        yield f"event: end\ndata: Stream finished due to auth error.\n\n"
        return

    if not task_data: # Should be caught by caller `stream_cv_progress` but double check
        yield f"data: {json.dumps({'type': 'error', 'message': 'Processing task data not found.'})}\n\n"
        yield f"event: end\ndata: Stream finished due to missing task data.\n\n"
        return
        
    # This check handles cases where stream is re-opened for an already processed/running task
    ...
    task_data["status"] = "running" # Update the shared task_data
    # `token` was already in task_data, use it directly
    # github_api_token = task_data["token"] # Redundant if github_pat_token is passed correctly
    ...
    try:
        async for event in orchestrate_cv_generation_for_repos(github_pat_token, repos_to_process, action_type):
            ...
            if event.get("type") == "complete":
                task_data["results"] = event.get("final_results", []) # Update shared task_data
                task_data["status"] = "complete"                     # Update shared task_data
            await asyncio.sleep(0.01) 

    except asyncio.CancelledError:
         print(f"Stream for process {process_id} cancelled (client disconnected).")
         task_data["status"] = "cancelled" # Update shared task_data
        ...
        task_data["status"] = "error" # Update shared task_data
...
async def stream_cv_progress(
    ...
    # Perform initial checks that don't require being part of the SSE stream itself
    if not github_pat:
        # This case should ideally not be hit if Depends works, but as a fallback
        # For a non-streaming error, a normal HTTP response is better
    ...
    # Pass necessary details to the generator.
    # app.state.processing_tasks is passed so the generator can update the central store.
    event_generator = _cv_event_stream_generator(
        process_id=process_id,
        github_pat_token=github_pat, # Pass the validated PAT
        app_state_tasks=app.state.processing_tasks # Pass the whole dict for updates
    )
